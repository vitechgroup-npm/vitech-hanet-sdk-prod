//#region src/sdk/core/bodySerializer.gen.ts
var e = (e, t, n) => {
	typeof n == "string" || n instanceof Blob ? e.append(t, n) : n instanceof Date ? e.append(t, n.toISOString()) : e.append(t, JSON.stringify(n));
}, t = (e, t, n) => {
	typeof n == "string" ? e.append(t, n) : e.append(t, JSON.stringify(n));
}, n = { bodySerializer: (t) => {
	let n = new FormData();
	return Object.entries(t).forEach(([t, r]) => {
		r != null && (Array.isArray(r) ? r.forEach((r) => e(n, t, r)) : e(n, t, r));
	}), n;
} }, r = { bodySerializer: (e) => JSON.stringify(e, (e, t) => typeof t == "bigint" ? t.toString() : t) }, i = { bodySerializer: (e) => {
	let n = new URLSearchParams();
	return Object.entries(e).forEach(([e, r]) => {
		r != null && (Array.isArray(r) ? r.forEach((r) => t(n, e, r)) : t(n, e, r));
	}), n.toString();
} };
Object.entries({
	$body_: "body",
...
//#endregion
//#region src/sdk/core/serverSentEvents.gen.ts
function a({ onRequest: e, onSseError: t, onSseEvent: n, responseTransformer: r, responseValidator: i, sseDefaultRetryDelay: a, sseMaxRetryAttempts: o, sseMaxRetryDelay: s, sseSleepFn: c, url: l, ...u }) {
...
//#endregion
//#region src/sdk/core/pathSerializer.gen.ts
var o = (e) => {
	switch (e) {
		case "label": return ".";
		case "matrix": return ";";
...
//#endregion
//#region src/sdk/core/auth.gen.ts
var ee = async (e, t) => {
...
}, v = (e, t) => t ? !!(e.headers.has(t) || e.query?.[t] || e.headers.get("Cookie")?.includes(`${t}=`)) : !1;
async function te(e) {
	for (let t of e.security ?? []) {
		if (v(e, t.name)) continue;
		let n = await ee(t, e.auth);
		if (!n) continue;
...
var y = (e) => m({
	...
	path: e.path,
	query: e.query,
	querySerializer: typeof e.querySerializer == "function" ? e.querySerializer : g(e.querySerializer),
	url: e.url
}), b = (e, t) => {
	let n = {
		...e,
...
}), ie = { "Content-Type": "application/json" }, w = (e = {}) => ({
	...r,
	headers: ie,
	parseAs: "auto",
	querySerializer: re,
	...
	let t = b(w(), e), n = () => ({ ...t }), r = (e) => (t = b(t, e), n()), i = ne(), o = async (e) => {
		let n = {
			...t,
			...e,
		...
		n.security && await te(n), n.requestValidator && await n.requestValidator(n), n.body !== void 0 && n.bodySerializer && (n.serializedBody = n.bodySerializer(n.body)), (n.body === void 0 || n.serializedBody === "") && n.headers.delete("Content-Type");
		let r = n;
		return {
			opts: r,
			url: y(r)
		...
		let n = e.throwOnError ?? t.throwOnError, r = e.responseStyle ?? t.responseStyle, a, s;
		try {
			...
			let c = t.fetch;
			s = await c(a);
			for (let e of i.response.fns) e && (s = await e(s, a, t));
			let l = {
				request: a,
				response: s
			};
			if (s.ok) {
				let e = (t.parseAs === "auto" ? _(s.headers.get("Content-Type")) : t.parseAs) ?? "json";
			...
			let o = t;
			for (let t of i.error.fns) t && (o = await t(o, s, a, e));
			if (o ||= {}, n) throw o;
...
}, E = T(w({ baseUrl: "https://partner.hanet.ai" })), D = (e) => (e.client ?? E).get({
...
function Q(e) {
	e.interceptors.request.use((e) => {
		let t = e.url;
		if ((t.includes("/token") || t.includes("/oauth2/authorize")) && !t.startsWith("https://oauth.hanet.com")) try {
			let n = t;
			if (t.startsWith("http")) {
				let e = new URL(t);
				n = e.pathname + e.search;
			}
			let r = `https://oauth.hanet.com${n}`;
...
var $ = class {
	constructor(e, t) {
		this.isRefreshing = null, this.credentials = e, this.storage = t?.storage || new Z(), this.fallbackClient = t?.fallbackClient || T({ baseUrl: "https://partner.hanet.ai" }), Q(this.fallbackClient);
	}
	async setToken(e) {
		e.expires_in && !e.expires_at && (e.expires_at = Date.now() + e.expires_in * 1e3), await this.storage.setToken(e);
	}
	async getToken() {
		return await this.storage.getToken();
	}
	async isTokenExpired(e = 3e5) {
		let t = await this.getToken();
		return !t || !t.access_token ? !0 : t.expires_at ? Date.now() + e >= t.expires_at : !1;
	}
	async refreshAccessToken() {
		return this.isRefreshing ||= (async () => {
			try {
				let e = await this.getToken();
				if (!e || !e.refresh_token) return console.warn("[HANET SDK] Không tìm thấy refresh_token để tự động refresh."), null;
				...
				if (t && t.access_token) {
					let n = {
						...
						refresh_token: t.refresh_token || e.refresh_token,
						expires_in: t.expires_in ? Number(t.expires_in) : 86400
					};
					return await this.setToken(n), console.log("[HANET SDK] Tự động refresh token thành công!"), n;
				} else return console.error("[HANET SDK] Refresh token thất bại. Response:", JSON.stringify(t)), null;
			} catch (e) {
				return console.error("[HANET SDK] Lỗi nghiêm trọng khi refresh token:", e), null;
			} finally {
	...
	async getOrRefreshAccessToken() {
		...
		if (await this.isTokenExpired()) {
			let e = await this.refreshAccessToken();
	...
	static createClient(e, t) {
		let n = T({ baseUrl: t?.baseUrl || "https://partner.hanet.ai" });
		Q(n);
		let r = new $(e, {
			storage: t?.storage,
...
//#endregion
export { ze as HanetClientFactory, $ as HanetTokenManager, Z as MemoryTokenStorage, E as client, Le as departmentAddPerson, Pe as departmentCreate, Me as departmentList, Ne as departmentListPerson, Ie as departmentRemove, Re as departmentRemovePerson, Fe as departmentUpdate, q as deviceGetConnectionStatus, K as deviceGetConnectionStatusDeprecated, V as deviceGetDeviceInfo, B as deviceGetListDevice, U as deviceGetListDeviceByPlace, H as deviceGetListDeviceByPlaceDeprecated, z as deviceGetListDeviceDeprecated, G as deviceSetDeviceMqtt, W as deviceUpdateDevice, D as oauth2Authorize, A as partnerAddPlacePartner, N as partnerGetListUserPartner, j as partnerRemovePlacePartner, M as partnerRemoveUserPartner, k as partnerUpdateToken, Oe as personGetCheckinByPlaceIdInDay, Ae as personGetCheckinByPlaceIdInTimestamp, ge as personGetListByAliasId, he as personGetListByAliasIdAllPlace, me as personGetListByAliasIdAllPlaceDeprecated, fe as personGetListByPlace, de as personGetListByPlaceDeprecated, ke as personGetTotalCheckinByPlaceIdInDay, X as personGetTotalCheckinByPlaceIdInTimestamp, pe as personGetTotalPersonByPlaceId, _e as personGetUserInfoByAliasId, ve as personGetUserInfoByPersonId, J as personRegister, Y as personRegisterByUrl, ye as personRemove, Se as personRemoveAllPersonInPlace, be as personRemoveByPlace, Ce as personRemovePersonById, xe as personRemovePersonByListAliasId, De as personTakeFacePicture, we as personUpdate, Ee as personUpdateAliasId, ae as personUpdateByFaceImage, oe as personUpdateByFaceImageByAliasId, se as personUpdateByFaceImageByPersonId, ce as personUpdateByFaceUrl, le as personUpdateByFaceUrlByAliasId, ue as personUpdateByFaceUrlByPersonId, Te as personUpdateInfo, F as placeAddPlace, R as placeGetPlaceInfo, P as placeGetPlaces, L as placeRemovePlace, I as placeUpdatePlace, je as profileGetProfile, Q as registerOAuth2UrlInterceptor, O as token };

//# sourceMappingURL=vitech-hanet-sdk.js.map